...
class Producto {
    constructor(datos){
      ...
      this.medida = datos.medida;
      this.metrosCaja = datos.metrosCaja
      this.cantidad = 1;
    }
    addCantidad(){//dasdsad
      this.cantidad++;
    ...
    getTotalMetrosCuadrados(){
      return(this.cantidad * this.metrosCaja);
    }
  }
  ...
  $(document).ready(function () {
    $(".btn-producto").click(function (e) { 
      ...
      for (const producto of SELECCIONADOS)  {
        $("#compras").append(`<h3><strong>${producto.nombre}</strong></h3> 
        <h3>Cantidad ${producto.cantidad} Total: $${producto.getTotal()}</h3>
        <h3>Metros totales: ${producto.getTotalMetrosCuadrados()}mts²</h3>`);
    }
    });
  });
  //----------------------------------PROGRAMA PRINCIPAL----------------------------------//
...
//Implementamos el uso de AJAX, PARA LA FUNCIONALIDAD SINCRONICA UTILIZADA
 ...
 $(document).ready($.getJSON("ceramicaPared.json",function(datos,estado){
    console.log(datos);
    console.log(estado);
  }))
